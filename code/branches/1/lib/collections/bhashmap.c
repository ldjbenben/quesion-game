...
bhashmap_t lc_bhashmaps[BHASHMAP_MAX_NUM] = {{0}};
bmemory_pool_id_t lc_pool_id_pentry = 0;
...
void bhashmap_init(void)
{
	...
	lc_pool_id_value = bmemory_pool_register(1, 20480, 20480);
}
...
bhashmap_id_t bhashmap_register(void)
{
	...
	if(id == BHASHMAP_MAX_NUM)
	{
		printf("bhashmap max num is %d\n", BHASHMAP_MAX_NUM);
		exit(1);
	}
	...
	for(i=0; i<lc_bhashmaps[id].capacity; i++)
	{
		*(lc_bhashmaps[id].table + i) = NULL;
	}
...
void bhashmap_unregister(bhashmap_id_t id)
{
	...
	// 释放内存
	int i;
	bhashmap_entry_t* entry;
	
	for(i=0; i<bhashmap->capacity; i++)
	{
		entry = *(bhashmap->table + i);
		while(entry != NULL)
		{
			bmemory_free(lc_pool_id_value, entry->value);
			bmemory_free(lc_pool_id_entry, entry);
			entry = entry->next;
		}
	}
	
	bmemory_free(lc_pool_id_pentry, bhashmap->table);
...
void bhashmap_set(bhashmap_id_t id, const char* key, void* value, bhashmap_value_size_t value_len)
{
	bhashmap_t* bhashmap = _get_bhashmap(id);
	...
	bhashmap_size_t index;	
	bhashmap_entry_t* entry = _find_entry(bhashmap, key, &index);
...
void* bhashmap_get(bhashmap_id_t id, const char* key)
{
	...
	bhashmap_entry_t* entry = _find_entry(bhashmap, key, NULL);
...
void bhashmap_unset(bhashmap_id_t id, const char* key)
{
	...
	if(entry != NULL)
	{
		if(entry == bhashmap->table[index])
		{
			bhashmap->table[index] = entry->next;
		}
		else
		{
			if(entry->next != NULL)
...
static bhashmap_t* _get_bhashmap(bhashmap_id_t id)
{
	id--;
	bhashmap_t* bhashmap = &lc_bhashmaps[id];
	
	if(bhashmap->capacity == 0 || bhashmap->table == NULL)
	{
		bhashmap = NULL;
	}
 ...
 */ 
static unsigned int _find_hash_index(bhashmap_t* m, const char* keystring)
{ 
...
void bhashmap_report(void)
{
	int id;
        int i;
        bhashmap_t* bhashmap = NULL;
        bhashmap_entry_t* node = NULL;
        printf("\n\n===================数据报告=================\n\n");
		...
		for(i=0; i<bhashmap->capacity; i++)
		{
			...
			if(node != NULL)
			{
				printf("\n");
              	 	...
              	 	while(node != NULL)
              		{
	                       	printf("\t%d:[key:%s,value:%p]", i, node->key, node->value);
                        	node = node->next;
        	        }
		}
        
	        printf("\n节点容量：%d\t数据条数：%d\n", bhashmap->capacity, bhashmap->size);
        }
